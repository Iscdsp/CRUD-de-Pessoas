...
const LS_CHAVE_EST: string = 'estados';
@Injectable({
  providedIn: 'root',
})
export class EstadoService {
  constructor() {}
  listarTodos(): Estado[] {
    const estados = localStorage[LS_CHAVE_EST];
  ...
  inserir(estado: Estado): void {
    ...
    estados.push(estado);
    localStorage[LS_CHAVE_EST] = JSON.stringify(estados);
  }

  buscarPorId(id: number): Estado | undefined {
    const estados: Estado[] = this.listarTodos();
    return estados.find((estados) => estados.id === id);
  }
